...
class FancyPlot():
    ...
    def stripe_from_data(self, data, start_i, end_i, x_key = 'timestamp', ax_id = 0, color = '0.9', datetime = False, timezone = 'Europe/Stockholm'):
        start = data.df[x_key].iloc[start_i]
        end = data.df[x_key].iloc[end_i]
        self.stripe(start, end, ax_id = ax_id, color = color, datetime = datetime, timezone = timezone)


    def stripe(self, start_x, end_x, ax_id = 0, color = '0.9', datetime = False, timezone = 'Europe/Stockholm'):
        if datetime:
    ...
    def stripe_files(self, data, x_key = 'timestamp', ax_id = 0, color = '0.9', sec_color = '1.0', datetime = False, timezone = 'Europe/Stockholm'):
        f_sep = data.file_separators

        for i in range(f_sep.shape[0]):
            current_color = color if i%2 else sec_color
            self.stripe_from_data(data, f_sep[i,0], f_sep[i,1], x_key = x_key, ax_id = ax_id, color = current_color, datetime = datetime, timezone = timezone)
    ...
    def plot_datetime(self, x, y, ax_id = 0, date_format = "%m-%d %H:%M:%S", timezone = 'Europe/Stockholm', marker = None, \
    markersize = 5, linestyle = '-', linewidth = 2, color = None, label = None, scaling_x = 1, scaling_y = 1):
        ...
        plt.setp(self.axs[ax_id].xaxis.get_majorticklabels(), rotation=70)

        self.plot(new_x,  y, scaling_x = 1, scaling_y = scaling_y, ax_id = ax_id, color = color, marker = marker, markersize = markersize, \
            linestyle = linestyle, linewidth = linewidth, label = label)


    def plot_data(self, data, keys, x_key = 'timestamp', plot_datetime = True, date_format = "%m-%d %H:%M:%S", timezone = 'Europe/Stockholm', \
    ax_id = None, marker = None, markersize = 5, linestyle = '-', linewidth = 2, color = None, labels = None, scaling_x = 1, scaling_y = 1, use_style_dict = True):
        ...
        for i in range(0, n_axes):
            ...
            if plot_datetime and x_key == 'timestamp':
                ...
                plt.setp(current_ax.xaxis.get_majorticklabels(), rotation=70)
            ...
            for j in range(0, len(keys[i])):
                ...
                if labels is None:
                    label = data.label_dict[keys[i][j]]
                else:
                    label = labels[i][j]
                ...
                self.plot(new_x,  y_data, ax_id = current_ax_id, scaling_x = scaling_x, scaling_y = scaling_y, color = current_color, marker = current_marker, \
                    markersize = current_markersize, linestyle = current_linestyle, linewidth = current_linewidth, label = label)
